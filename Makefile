...
IMG ?= quay.io/pdettori/kealm:latest
JOB_IMG ?= quay.io/pdettori/vksbuilder:latest
...
TMP_DIR := $(shell mktemp -d -t vks-XXXXXXXXXX)
...
docker-load-job: ## Load vks image in kind
	kind load docker-image ${JOB_IMG} --name ${KIND_CLUSTER_NAME}

.PHONY: docker-push-job
docker-push-job: ## Push docker image for vksjob.
	docker push ${JOB_IMG}
